        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(db.rst.next())
            {
                if(npswd.equals(cpswd))
                {
                    ...
                    if(i>0)
                    {
                        response.sendRedirect("ChangePasswordAdmin.jsp?success=Password Updated Successfully");
                    }
                }
                else
                {
                    response.sendRedirect("ChangePasswordAdmin.jsp?error=New and Confirm Password must be Same");
                }
            }
            else
            {
                response.sendRedirect("ChangePasswordAdmin.jsp?error=Old Password does not Correct");
            }
        }
        catch(Exception e)
        {
